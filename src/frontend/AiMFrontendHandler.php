...
class AiMFrontendHandler {

    // Continuing AIM_Shortcodes class from above...

public function handle_frontend_ajax_action() {
    check_ajax_referer( 'aim_frontend_nonce', '_ajax_nonce' );
    ...
    $api_handler = AIM_API_Handler::instance(); // Make sure this class is loaded
    ...
    switch ( $sub_action ) {
        case 'start_mastering':
            ...
            delete_post_meta( $post_id, '_aim_job_id' );
            delete_post_meta( $post_id, '_aim_report_data' );
            delete_post_meta( $post_id, '_aim_report_message' );
            update_post_meta( $post_id, '_aim_status', 'pending_submission' );


            // Parameters for mastering (get from plugin settings or defaults)
            $mastering_params = [
                'target_loudness' => (float) get_option( 'aim_target_loudness', -10.0 ),
            ...
            if ( is_wp_error( $job_id_or_error ) ) {
                ...
                update_post_meta( $post_id, '_aim_report_message', $job_id_or_error->get_error_message() );
                wp_send_json_error( ['message' => $job_id_or_error->get_error_message()] );
            } else {
                update_post_meta( $post_id, '_aim_job_id', $job_id_or_error );
                update_post_meta( $post_id, '_aim_status', 'submitted_processing' );
                update_post_meta( $post_id, '_aim_report_message', 'Submitted to AI Mastering. Job ID: ' . $job_id_or_error );
        ...
        case 'refresh_status':
            ...
            if ( is_wp_error( $report_or_error ) ) {
                update_post_meta( $post_id, '_aim_status', 'error_fetching_status' );
                // Don't overwrite good report data with an error, but update message
                update_post_meta( $post_id, '_aim_report_message', 'Error refreshing status: ' . $report_or_error->get_error_message() );
                ...
                update_post_meta( $post_id, '_aim_status', $new_status );
                update_post_meta( $post_id, '_aim_report_data', $report_or_error['report_data'] ); // Store the full sub-report
                ...
                update_post_meta( $post_id, '_aim_report_message', $message);
    ...
    // In AIM_Shortcodes::handle_frontend_ajax_action()
// ...
    switch ( $sub_action ) {
        case 'start_mastering':
            // ... (existing code for the button in [aim_mastering_status]) ...
            break;
        ...
        case 'submit_mastering_with_options': // New case for the form
            ...
            // Based on Aimastering.MasteringApi.createMastering documentation:
            ...
            delete_post_meta( $post_id, '_aim_report_data' );
            delete_post_meta( $post_id, '_aim_report_message' );
            update_post_meta( $post_id, '_aim_status', 'pending_submission_form' ); // Differentiate if needed
            ...
            if ( is_wp_error( $job_id_or_error ) ) {
                update_post_meta( $post_id, '_aim_status', 'submission_failed_form' );
                update_post_meta( $post_id, '_aim_report_message', $job_id_or_error->get_error_message() );
                wp_send_json_error( ['message' => $job_id_or_error->get_error_message()] );
            } else {
                update_post_meta( $post_id, '_aim_job_id', $job_id_or_error );
                update_post_meta( $post_id, '_aim_status', 'submitted_processing_form' );
                update_post_meta( $post_id, '_aim_report_message', 'Submitted via form. Job ID: ' . $job_id_or_error );
                wp_send_json_success( [
...
public function handle_download_mastered_file() {
    ...
    check_ajax_referer('aim_download_mastered_file_' . $post_id . '_' . $output_audio_id, '_ajax_nonce');
    ...
    $report_data = get_post_meta($post_id, '_aim_report_data', true);
    ...
    // This method needs to be created in AIM_API_Handler
    ...
    if ($direct_download_url) {
        ...
        // Example of initiating a stream if your AIM_API_Handler had such a method:
