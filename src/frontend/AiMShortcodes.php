<?php
namespace SparxstarAiMastering\src\frontend;
// In includes/class-aim-shortcodes.php (or main plugin file)
...
class AIMShortcodes {
    ...
    private function __construct() {
        add_shortcode( 'aim_mastering_status', [ $this, 'render_mastering_status_shortcode' ] );
        add_action( 'wp_enqueue_scripts', [ $this, 'enqueue_frontend_assets' ] );

        // AJAX action for frontend interactions
        add_action( 'wp_ajax_aim_frontend_action', [ $this, 'handle_frontend_ajax_action' ] );
        add_action( 'wp_ajax_nopriv_aim_frontend_action', [ $this, 'handle_frontend_ajax_action' ] ); // For non-logged-in users if applicable

        // AJAX action for download (could also be a REST API endpoint)
        add_action( 'wp_ajax_aim_download_mastered_file', [ $this, 'handle_download_mastered_file' ] );
        add_action( 'wp_ajax_nopriv_aim_download_mastered_file', [ $this, 'handle_download_mastered_file' ] );
    }
    ...
    public function enqueue_frontend_assets() {
        ...
        global $post;
        if ( is_a( $post, 'WP_Post' ) && (has_shortcode( $post->post_content, 'aim_mastering_status' ) || get_post_type($post) === 'music_track' /* your CPT */) ) {
            wp_enqueue_style( 'aim-frontend-style', AIM_PLUGIN_URL . 'assets/css/frontend-style.css', [], AIM_VERSION ); // Define AIM_VERSION
            wp_enqueue_script( 'aim-frontend-script', AIM_PLUGIN_URL . 'assets/js/frontend-script.js', ['jquery'], AIM_VERSION, true );
            wp_localize_script( 'aim-frontend-script', 'aim_ajax_object', [
                'ajax_url' => admin_url( 'admin-ajax.php' ),
                'nonce'    => wp_create_nonce( 'aim_frontend_nonce' ),
                // 'download_nonce' => wp_create_nonce( 'aim_download_nonce'), // If using separate nonce for download
            ]);
        }
    }
    ...
    public function render_mastering_status_shortcode( $atts ) {
        ...
        if ( ! $post_id || get_post_type( $post_id ) !== 'music_track' /* <<< YOUR CPT SLUG HERE */ ) {
            if (current_user_can('edit_posts')) { // Show error to admin/editor
                return '<p class="aim-error">AI Mastering: Invalid post ID or wrong post type for shortcode.</p>';
            }
        ...
        $job_id         = get_post_meta( $post_id, '_aim_job_id', true );
        $status         = get_post_meta( $post_id, '_aim_status', true );
        $report_data    = get_post_meta( $post_id, '_aim_report_data', true ); // This is the full API response from GET /masterings/{id}
        $report_message = get_post_meta( $post_id, '_aim_report_message', true );
        $audio_file_id  = get_post_meta( $post_id, '_audio_attachment_id', true ); // Or your file path meta key

        ob_start();
        ?>
        <div class="aim-status-wrapper" data-postid="<?php echo esc_attr( $post_id ); ?>">
            ...
            <?php if ( empty( $status ) || empty( $job_id ) ) : ?>
                <?php if ( ! empty( $audio_file_id ) ) : ?>
                    ...
                    <button class="aim-button aim-start-mastering-btn">Start AI Mastering</button>
                <?php else : ?>
                    <p>No audio file found for this track. Please upload an audio file first.</p>
                <?php endif; ?>

            <?php else : ?>
                <div class="aim-status-current">
                    <p><strong>Status:</strong> <span class="aim-status-text"><?php echo esc_html( ucfirst( str_replace( '_', ' ', $status ) ) ); ?></span></p>
                    <?php if ( $job_id ) : ?>
                ...
                <?php if ( in_array( $status, ['submitted_processing', 'processing'] ) ) : ?>
                    <button class="aim-button aim-refresh-status-btn">Refresh Status</button>
                <?php endif; ?>

                <?php if ( $status === 'completed' && ! empty( $report_data ) ) : ?>
                    <h4>Mastering Report:</h4>
                    <pre class="aim-report-data"><?php echo esc_html( print_r( $report_data, true ) ); ?></pre>
                    <?php
                    // --- Download Link Logic ---
                    ...
                    if ( $output_audio_id ) {
                        $ajax_download_url = add_query_arg([
                            'action' => 'aim_download_mastered_file',
                            'post_id' => $post_id,
                            'output_audio_id' => $output_audio_id,
                            '_ajax_nonce' => wp_create_nonce('aim_download_mastered_file_' . $post_id . '_' . $output_audio_id) // Specific nonce
                        ], admin_url('admin-ajax.php'));
                        echo '<a href="' . esc_url( $ajax_download_url ) . '" class="aim-button aim-download-btn" target="_blank">Download Mastered Track</a>';
                    } elseif ($download_url) {
                         echo '<a href="' . esc_url( $download_url ) . '" class="aim-button aim-download-btn" target="_blank" rel="noopener noreferrer">Download Mastered Track (Direct)</a>';
                    } else {
                ...
                <?php elseif ( in_array($status, ['submission_failed', 'failed', 'error_fetching_status'] ) ) : ?>
                    <p class="aim-error">There was an issue with the mastering process.</p>
                    <?php if ( ! empty( $audio_file_id ) ) : ?>
                        <button class="aim-button aim-start-mastering-btn aim-retry-btn">Retry Mastering</button>
                    <?php endif; ?>
                <?php endif; ?>
            <?php endif; ?>
            <div class="aim-ajax-message" style="display:none;"></div>
        </div>
        <?php
        return ob_get_clean();
    }
...
$('#aim-mastering-options-form').on('submit', function(e) {
    e.preventDefault();
    const $form = $(this);
    const $button = $form.find('.aim-submit-mastering-options-btn');
    const $wrapper = $form.closest('.aim-mastering-form-wrapper');
    ...
    $messageDiv.hide().removeClass('aim-error aim-success').empty();
    ...
    $.ajax({
        url: aim_ajax_object.ajax_url, // Already localized
        type: 'POST',
        data: formData,
        success: function(response) {
            if (response.success) {
                $form.hide();
                $messageDiv.text('Mastering process initiated successfully! Job ID: ' + (response.data.job_id || 'N/A') + '. You will be redirected to the status page shortly.').addClass('aim-success').show();
                // Redirect to status page or refresh.
                // For now, let's assume the status page URL could be the current page if [aim_mastering_status] is also there,
                // or a specific status page.
                setTimeout(function() {
                    // Option 1: Reload current page to show status via [aim_mastering_status] shortcode
